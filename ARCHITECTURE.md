...
The same route layer exposes top-level mobile destinations for statutes, the LCO index, the Judicial Branch infractions schedule, search, and device-local bookmarks. A responsive application shell presents these as persistent bottom navigation on narrow viewports and header navigation on larger screens. Index letter routes render the already-loaded headings as collapsed disclosure groups with level-indented entries; structured cross-reference routes retain exact heading and optional subheading targets and open the matching group without requiring another data service. Bookmarks and reader preferences—including whether repealed provisions appear in chapter navigation—use guarded browser storage only; they are never canonical data and never leave the device.
...
`ci.yml` verifies pull requests, pushes, and explicitly dispatched checks for automation-created branches. `deploy-pages.yml` performs the same checks, generates the installable application shell, discovery pages, sitemap, robots metadata, service worker, and static data into `dist/`, then deploys that directory through GitHub's official Pages Actions. `refresh-corpus.yml` and `refresh-secondary.yml` are weekly and manually dispatchable build-time workflows. Both acquire into staging, retain replayable source evidence as temporary artifacts, validate and diff candidates, apply versioned safety policies, and create draft data pull requests only for meaningful passing changes. A changed primary corpus re-resolves the published secondary records offline against the candidate base, preserving their source provenance and including a reviewed link diff. Only the independent secondary workflow downloads fresh PDFs; it uses Ubuntu system curl with verified HTTPS for the Judicial schedule. Scheduled and manual runs are serialized within each workflow, and neither writes directly to `main`.
