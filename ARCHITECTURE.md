...
The same route layer exposes top-level mobile destinations for statutes, the LCO index, the Judicial Branch infractions schedule, search, and device-local bookmarks. A responsive application shell presents these as persistent bottom navigation on narrow viewports and header navigation on larger screens. Index letter routes render the already-loaded headings as a continuous, level-indented page; structured cross-reference routes retain exact heading and optional subheading targets without requiring another data service. Bookmarks and reader preferences—including whether repealed provisions appear in chapter navigation—use guarded browser storage only; they are never canonical data and never leave the device.
