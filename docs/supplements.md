...
4. A partial overlap with a grouped base provision fails import and validation because the intended replacement is ambiguous.
...
`npm run build` derives `dist/data/supplements/manifest.json`, including an empty edition list when no supplements are checked in. `SupplementRepository` in `src/supplements.js` provides the client-side edition and chapter loading foundation.
...
The current Phase 6 scope establishes this contract, importer, validation, build index, and client loading/application primitives. Reader controls and supplement-aware search should be added only after a real edition has passed this review path.
